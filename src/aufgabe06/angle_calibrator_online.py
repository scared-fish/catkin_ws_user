...
sample_count = 150  # number RANSAC samples to take
...
def find_best_params(points):
    ...
    best_slope, best_intercept = best_params
    ...
    for _ in xrange(sample_count):
        ...
        if inl_count > second_count and slope*best_slope>=-1.1 and slope*best_slope<=-0.9:
            second_count = inl_count
            second_params = (slope, intercept)
    ...
    inlier_points_2 = points[np.where(get_inliers(points, *second_params))]
    ...
    slope2, intercept2 = second_params
...
def scan_callback(scan_msg):
    ...
    slope, intercept, slope2, intercept2 = find_best_params(points)  # detect a line in these coordinates
    ...
    if ((wall_angle>0)and (wall_angle<np.pi/2)):
        ...
        wall_angle +=np.pi/2
    wall_dist = abs(intercept)/pow(pow(slope,2)+1.0,0.5)  # shortest distance from current position to the first wall
    wall_dist2 = abs(intercept2)/pow(pow(slope2,2)+1.0,0.5)  # shortest distance from current position to the second wall
    ...
    if initial_line is None or scan_msg.header.stamp < initial_line.stamp:
        ...
        print 'starting to drive: wall_dist: %.3f, wall_dist2: %.3f, wall_angle: %.1f ' % (wall_dist, wall_dist2, np.rad2deg(wall_angle))
        if not manual_mode:
        ...
        if (abs_theta >0.06) and (abs_theta<1.2) and (speed<0):
            ...
            print('current wall dist: %.3f, current wall dist2: %.3f, dist diff: %.3f, angle diff: %.1f, current angle: %.1f, start angle: %.1f, turn radius: %.3f' %
              (wall_dist,wall_dist2, dist_diff,np.rad2deg(theta), np.rad2deg(wall_angle), np.rad2deg(initial_line.wall_angle), turn_radius))
        else:
            ...
            if (125>steering_angle and steering_angle>61):
                if (wall_dist<1.2):
                    if (len(turn_radii)>0):
                        ...
                        y=np.arctan(0.26/(pow(average_r,2)-pow(0.2,2)))
                        print ('steering angle: %.3f' % y)
                        l = 0.26        # 26cm
    ...
    if plotting:
        ...
        # plot inliers detected by ransac
        inlier_mask2 = get_inliers(points, slope2, intercept2)
        ax_a.scatter(*points[np.where(inlier_mask2)].T, color='r')
        ax_a.scatter(*points[np.where(np.logical_not(inlier_mask2))].T, color='b')

        line_x2 = np.linspace(-plt_window, plt_window, 10)
        line_y2 = intercept2 + line_x * slope2
        ax_a.plot(line_x2, line_y2, color='y')

        # draw coordinate system
        ax_a.axvline(0, color='k') #20, 26
        ax_a.axhline(0, color='k')
