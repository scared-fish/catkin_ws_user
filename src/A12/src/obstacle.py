...
def scan_callback(data):
	...
	# in den weiteren Rechnungen aber beruecksichtigt
	# inf -> 0
	# alles ueber 1,5m -> 0
...
def findRelevantObstacles(A,laneID): #Pruefen, ob es auf einer Bahn Hindernisse gibt
	...
	for i in A:
		...
		if dist <=13:#Falls der Punkt maximal 15 Zentimeter vom Zentrum der Bahn entfernt ist, ist das Obstacle relevant
			ret.append(ref)
	return (ret)
...
def findDistance(point1,point2,laneID,distance):#Distanz zwischen 2 Punkten auf der
	...
	point1=findPoint(point1,laneID)
	x1=point1[0]
	y1=point1[1]
	x2=point2[0]
	y2=point2[1]

	if laneID == 0: #Radius der Kreise, basierend auf der Bahn festlegen
		rad = 135
	else:
		rad = 165
	...
	if y1 <= 215: #Den Winkel von Punkt 1 auf dem Halbkreis berechnen (nutzlos, falls Punkt 1 auf keinem Halbkreis liegt.)
		beta = 90+(math.degrees(math.asin((215.0-y1)/rad)))
	else:
		beta = 90-(math.degrees(math.asin((y1-215.0)/rad)))

	if y2 <= 215: #Den Winkel von Punkt 2 auf dem Halbkreis berechnen (nutzlos, falls Punkt 2 auf keinem Halbkreis liegt.)
		alpha = 90+math.degrees(math.asin((215.0-y2)/rad))
	else:
		alpha = 90-math.degrees(math.asin((y2-215.0)/rad))
	...
	if x1 <= 194: #oberer Halbkreis
		...
		if x2 > 195: #Falls sich der 2.Punkt nicht im gleichen Bereich befindet
	    	...
	    	if 195 > x2 or x2 > 405 or y2 > 215: #Falls sich der 2.Punkt nicht im gleichen Bereich befindet
		...
		if 105 > x2 or x2 > 405 or y2 < 216: #Falls sich der 2.Punkt nicht im gleichen Bereich befindet
		...
		umf = (beta/180)*totUmf #Restumfang von Punkt 1 bis zum Ende des unteren Halbkreises
		if x2 < 406: #Falls sich der 2.Punkt nicht im gleichen Bereich befindet
			distance += umf
	...
	if newPoint1 != point2: #Wenn der neue berechnete Punkt nicht mit Punkt 2 uebereinstimmt, wird die Funktion erneut aufgerufen,
				#mit dem neuen Punkt als Punkt 1 und der bereits "zurueckgelegten" Distanz (Punkt 1 bis neuer Punkt 1)
...
""" Hier fuehren wir eine neue Logik ein:
